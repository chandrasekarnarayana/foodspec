...
from foodspec.harmonization import CalibrationCurve, apply_calibration, intensity_normalize_by_power, harmonize_datasets_advanced
...
def test_harmonize_advanced_residual_diag():
    wn = np.array([1000.0, 1010.0, 1020.0])
    spec1 = SpectralDataset(wn, np.array([[1.0, 2.0, 3.0]]), pd.DataFrame({"sample": [1]}), instrument_meta={"instrument_id": "A"})
    spec2 = SpectralDataset(wn + 1.0, np.array([[1.1, 2.1, 3.1]]), pd.DataFrame({"sample": [2]}), instrument_meta={"instrument_id": "B"})
