...
class TrustEvaluator:
    ...
    def fit_conformal(
        ...
        # Convert target_coverage to alpha (1 - coverage = alpha)
        alpha = 1.0 - self.target_coverage
        
        # Initialize conformal predictor
        condition_key = 'bin' if bins_cal is not None else None
        ...
        self._conformal.fit(y_cal, proba_cal, meta_cal=bins_cal)
    ...
    def evaluate(
        ...
        if self._calibrator_fitted and self._calibrator is not None:
            # TemperatureScaler uses predict(), IsotonicCalibrator uses transform()
            if hasattr(self._calibrator, 'predict'):
                proba_test_cal = self._calibrator.predict(proba_test)
            else:
                proba_test_cal = self._calibrator.transform(proba_test)
        else:
        ...
        # Abstention - use max set size as constraint
        max_set_size = max(cp_result.set_sizes) if cp_result.set_sizes else self._conformal._n_classes
        ...
        result = TrustEvaluationResult(
            ...
            accuracy_abstained=None,  # Not computed by evaluate_abstention
